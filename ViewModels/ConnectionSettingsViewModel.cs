...
namespace AttendanceShiftingManagement.ViewModels
{
    public sealed class ConnectionSettingsViewModel : ObservableObject
    {
        ...
        public bool ShowLanCredentialEditor => ShowCredentialEditor;
        public bool ShowFixedPresetNotice => false;
        ...
        public bool CanEditSelectedPresetCredentials => ShowCredentialEditor;
        ...
        public string HeaderDescription => IsSelectionOnly
            ? "Choose which app database is active for login, startup, dashboard data, and snapshots. You can test the selected preset here, then edit preset credentials later from System Settings."
            : "Select the active app database here. All preset credentials are editable from this screen, but the login form remains selection-only.";
        public string FooterDescription => IsSelectionOnly
            ...
            : "Save to apply the active app database and persist the selected preset credentials for this Windows user profile.";
        ...
        public string PresetHelpText => IsSelectionOnly
            ? "Use System Settings after login if you need to update Local, Network (LAN), or Remote credentials."
            : $"{CurrentPresetDisplayName} is editable here. Changes are stored in your Windows user profile and do not rewrite the shipped app defaults.";
        ...
        private void ExecuteSave()
        {
            ...
            if (ShowCredentialEditor && !TryUpdateSelectedPreset(showValidationErrors: true))
            {
                return;
            }
        ...
        private string BuildPresetLoadedMessage()
        {
            if (IsSelectionOnly)
            {
                return $"{CurrentPresetDisplayName} app database selected. Use System Settings to edit preset credentials.";
            }

            return $"{CurrentPresetDisplayName} is selected. Update its host and database credentials here, then save the preset.";
        }
