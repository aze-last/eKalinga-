...
namespace AttendanceShiftingManagement.Views
{
    ...
    {
        private readonly SeminarAttendanceViewModel _viewModel;
        private readonly DispatcherTimer _scanDebounceTimer;

        public SeminarAttendancePage(User currentUser)
        {
            InitializeComponent();
            _viewModel = new SeminarAttendanceViewModel(currentUser);
            ...
            SizeChanged += SeminarAttendancePage_SizeChanged;
        }
        ...
        private void SeminarAttendancePage_SizeChanged(object sender, SizeChangedEventArgs e)
        {
            if (_viewModel?.IsOnboardingOpen == true)
            {
                Dispatcher.BeginInvoke(DispatcherPriority.Loaded, () => UpdateSpotlight());
        ...
        {
            if (e.PropertyName == nameof(SeminarAttendanceViewModel.IsOnboardingOpen) ||
                e.PropertyName == nameof(SeminarAttendanceViewModel.OnboardingStep) ||
                e.PropertyName == nameof(SeminarAttendanceViewModel.SelectedEvent))
            {
                Dispatcher.BeginInvoke(DispatcherPriority.Loaded, () => UpdateSpotlight());
            }
        }
